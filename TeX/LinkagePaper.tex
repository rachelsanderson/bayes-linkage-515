...
\subsection{Priors} I use only flat priors for my analysis, $(\alpha_M=\beta_M=\alpha_{M\ell}=\beta_{M\ell}=\alpha_{U\ell}=\beta_{U\ell}=1$), however, my programs will also allow for more informed priors.  FUTURE VERSIONS MIGHT TRUNCATE. %TO DO (jupyter notebook?)  Some small scale simulations showed that my results were extremely sensitive/not sensitive to piror. 
